...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		List<Message> removeDupeList = removeDuplicatePeersBeforeVoting();
		// multicast the message to activenodes (hint: use multicastMessage)
		multicastMessage(message, removeDupeList);
		// check that all replicas have replied (permission)
		if (areAllMessagesReturned(removeDupeList.size())) {
			acquireLock();
		...
		// return permission
		boolean permission = areAllMessagesReturned(removeDupeList.size());
		return permission;
	}
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for (Message m : activenodes) {
			...
			Util.getProcessStub(m.getNodeIP(), m.getPort()).onMutexRequestReceived(m);
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		case 0: {
			...
			Util.getProcessStub(message.getNodeIP(), port).onMutexAcknowledgementReceived(message);
			// acknowledge message
			message.setAcknowledged(true);
